...
const ProductItem: React.FC<Product> = ({ image, name, category, slug }) => {
  const { onOpen, onProductDetail } = useProductModal();

  const handleQuickView = (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    e.stopPropagation();
    e.preventDefault();
    onProductDetail({ image, name, category });
    onOpen();
  };
      ...
      {pathName === '/product' ? (
        <div className='group overflow-hidden bg-white md:h-[265px]'>
          <div className='h-32 md:h-[185px] overflow-hidden relative'>
            <Image src={image} alt={name} width={500} height={500} className='object-cover transition  w-full h-full' />
            <div onClick={(e) => handleQuickView(e)} className='absolute bottom-0 w-full bg-[#D1D109] uppercase text-white text-center text-sm font-bold translate-y-10 transition-all duration-300 py-2 group-hover:translate-y-0'>
              quick view
            </div>
          </div>
