...
const BannerLink = () => {
  return (
    <div className='grid grid-cols-1 md:grid-cols-3 divide-y md:divide-x md:divide-y-0'>
      {images.map((image) => (
        <div key={image.imageAlt} className='w-full h-[250px] relative bg-cover bg-white overflow-hidden group' style={{ backgroundImage: `url(${image.imageUrl})` }}>
          <div className='absolute w-full top-0 left-0 bottom-0 right-0 flex flex-col gap-2 justify-center items-center bg-neutral-800/70 translate-y-[250px] transition-all duration-300 group-hover:translate-y-0'>
            <div className='text-sm md:text-lg text-white font-bold drop-shadow-lg shadow-black'>{image.imageTitle}</div>
